...
function renderUnsupportedStep(
  ...
  return [
    renderSpssStepComment(step, context.commentLanguage),
    spssComment(
...
function renderValueLabels(step: CleaningStep, context: RenderContext): string {
  const lines = [renderSpssStepComment(step, context.commentLanguage)]
  ...
  findStepVariables(step, context.variablesByName).forEach((variable) => {
    if (!variable.valueLabels || variable.valueLabels.length === 0) {
      ...
      lines.push(spssComment(rendererWarning(context.commentLanguage, message)))
      return
    }
      ...
      spssComment(
        rendererComment(
          ...
          'comment.declaredMissingCodes',
          {
            ...
            values: formatSpssList(values),
          },
        ),
      ),
      spssComment(
        rendererComment(context.commentLanguage, 'comment.spssMissingPreserve'),
      ),
...
function renderOutlierFlag(step: CleaningStep, context: RenderContext): string {
  ...
  findStepVariables(step, context.variablesByName).forEach((variable) => {
    ...
    if (method === 'mad' || method === 'mad_robust') {
      lines.push(
        ...renderMadOutlierTemplate(step, variable, context.commentLanguage),
      )
      return
    }
    ...
    if (method === 'tukey' || method === 'tukey_fences') {
      lines.push(
        ...renderTukeyOutlierTemplate(step, variable, context.commentLanguage),
      )
      return
    }
    ...
    lines.push(
      spssComment(rendererWarning(context.commentLanguage, unsupportedMessage)),
    )
  })
...
function renderStructuralMissingCheck(
  ...
  lines.push(
    spssComment(rendererWarning(context.commentLanguage, partialMessage)),
  )
...
function renderImputation(step: CleaningStep, context: RenderContext): string {
  const lines = [
    ...
    spssComment(
      rendererComment(context.commentLanguage, 'comment.spssImputationReview'),
    ),
    spssComment(
      rendererComment(
        context.commentLanguage,
  ...
  lines.push(
    ...
    spssComment(
      rendererComment(context.commentLanguage, 'comment.spssImputationInspect'),
    ),
  )
...
function renderAuditLog(step: CleaningStep, context: RenderContext): string {
  ...
  context.warnings.push(`Step "${step.id}": ${message}`)
  lines.push(
    spssComment(rendererWarning(context.commentLanguage, message)),
    spssComment(
...
function renderMadOutlierTemplate(
  ...
  return [
    spssComment(
      rendererComment(language, 'comment.spssMadPlaceholder', {
        variable: variable.name,
      }),
    ),
    spssComment(
      rendererComment(language, 'comment.spssMadThreshold', { threshold }),
    ),
    `NUMERIC ${flag} (F1.0).`,
    `COMPUTE ${flag} = 0.`,
    spssComment(
      rendererComment(language, 'comment.spssMadIf', {
        variable: variable.name,
        threshold,
        flag,
      }),
...
function renderTukeyOutlierTemplate(
  ...
  return [
    spssComment(
      rendererComment(language, 'comment.spssTukeyPlaceholder', {
        variable: variable.name,
      }),
    ),
    spssComment(
      rendererComment(language, 'comment.spssTukeyMultiplier', { multiplier }),
    ),
    `NUMERIC ${flag} (F1.0).`,
    `COMPUTE ${flag} = 0.`,
    spssComment(
      rendererComment(language, 'comment.spssTukeyIf', {
        variable: variable.name,
        flag,
      }),
    ),
